...
export interface MarkdownTransformContext {
	...
	/**
	 * Per-message identity carried from the session entry. Contract:
	 * - `messageId` is the ACTUAL persisted session-entry id. It is `undefined` for
	 *   new messages until the entry is persisted (message_end + appendMessage).
	 *   Components must never fabricate synthetic values for this field.
	 * - `timestamp` is the ISO timestamp from the session entry, same lifecycle as messageId.
	 * - `transientId` is a per-render-session stable id generated by the presentation
	 *   component. It stays constant across streaming updates and width changes within
	 *   one render session, letting extensions correlate frames of the same message
	 *   before the canonical entry id exists. It is NOT the session entry id.
	 */
	messageId?: string;
	timestamp?: string;
	transientId?: string;
